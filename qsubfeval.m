...
%
% $Id: qsubfeval.m 10649 2015-09-01 15:47:27Z roboos $
% -----------------------------------------------------------------------
...
%       submitoptions = [submitoptions sprintf(' -l mem=%.0fgb ',   memreq)];
      submitoptions = [submitoptions sprintf(' -l mem=%.0fmb ',   memreq)];
...
%       cmdline = sprintf('sbatch --job-name=%s %s --output=%s --error=%s %s -r "%s" ', jobid, submitoptions, logout, logerr, matlabcmd, matlabscript);
      cmdline = sprintf('sbatch --job-name=%s %s --output=%s --error=%s --wrap "%s -r \\"%s\\"" ', jobid, submitoptions, logout, logerr, matlabcmd, matlabscript);
    end
